#!/usr/bin/env bash
# One-shot LoCoMo eval flow for the NMG bridge: preflight (kill strays, pin
# env), run (or resume) the OmniMemEval locomo pipeline, then analyze.
# Mirror of run-lme.sh for the locomo dataset.
#
# Usage:
#   ./run-locomo.sh --env-file <file> --version <label> [options]
#   ./run-locomo.sh --analyze <results-dir>            # analyze an existing run
#
# Options:
...
kill_strays() {
    powershell.exe -NoProfile -Command \
        "Get-CimInstance Win32_Process | Where-Object { \$_.CommandLine -like '*evals\omnimemeval\bridge.ts*' -or \$_.CommandLine -like '*scripts\locomo\locomo_*' -or \$_.CommandLine -like '*run_locomo_eval.sh*' } | ForEach-Object { Stop-Process -Id \$_.ProcessId -Force -ErrorAction SilentlyContinue }" \
        >/dev/null 2>&1 || true
}
...
if [[ -n "$ANALYZE_DIR" ]]; then
    echo "== search evidence audit (official method) =="
    python "$SCRIPT_DIR/audit-locomo-official.py" "$ANALYZE_DIR/nmg_locomo_search_results.json" 2>/dev/null || true
    exit 0
fi
...
# Analyze
# ────────────────────────────────────────────────────────────────────────────
RESULT_DIR="$(ls -dt "$OMNI_DIR/results/locomo/"*"$VERSION"* 2>/dev/null | head -1 || true)"
if [[ -n "$RESULT_DIR" && -f "$RESULT_DIR/nmg_locomo_search_results.json" ]]; then
    echo "== search evidence audit (official method) =="
    python "$SCRIPT_DIR/audit-locomo-official.py" "$RESULT_DIR/nmg_locomo_search_results.json" 2>/dev/null || true
fi
