...
test("host-neutral action contracts are unique and Pi only adds host-owned actions", () => {
  ...
  assert.deepEqual(PI_BOARD_ACTIONS.filter((action) => !COMMON_BOARD_ACTIONS.includes(action as never)), ["rename"]);
  assert.deepEqual(PI_REMEMBER_ACTIONS.filter((action) => !COMMON_REMEMBER_ACTIONS.includes(action as never)), ["feedback"]);
});
...
test("Pi, MCP, and DSH schemas consume the shared action contract", () => {
  const sources = [
    ...
    ["../../claude-plugins/nmg-memory/agents/memory-copilot.ts", /COMMON_BOARD_ACTIONS/u, /COMMON_REMEMBER_ACTIONS/u],
