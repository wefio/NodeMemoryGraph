...
| `acceptedArtifacts()` | the verdict is looked up on the board by the digest of *this attempt's* artifact; no pointer column |
...
| mutation teeth | `node <shared-checkout>/.nmg/teeth-check-task-semantics.mjs` | 15 of 15 mutants caught by name across 4 targets, each restored byte-identically |
...
- **`prune-ignores-retention`** was not caught because the retention test only called the
  *global* prune; the channel-scoped path, which has its own DELETE, was never exercised. The
  case now calls both.
...
- `ooo_probe_tasks` still holds immutable input (`input`, `revision`), candidate bytes
  ...
  `accepted_entry_id`) in one table. This change removes one derived column's *use*; splitting the
...
node <shared-checkout>/.nmg/teeth-check-task-semantics.mjs   # local one-off, as in the earlier slices
```
