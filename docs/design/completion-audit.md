...
| Temporal/logical chains | Optional ordered/DAG references reuse exact memory IDs; temporal order needs explicit time, logical order needs attributable supervision; chain expansion cannot imply truth or causality | chain store/retrieval, exact-get hydration, STG/LTG edge merge, compact Pi projection, daemon protocol, CLI and chain tests | **Verified core** | Domain-specific automatic chain construction is a harness policy, not a Core default. |
