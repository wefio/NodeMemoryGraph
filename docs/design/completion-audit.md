...
| Repository Control Plane | External Contract-to-Agent-to-Code-to-Verifier reconciliation with separate desired, observed, receipt and forge truth domains; optional one-way NMG integration | feature-branch `src/rcp/` candidate, `nmg-rcp` CLI, shared context/verification primitives, deterministic RCP tests and paired proposed decision | **Designed target** | Candidate code is not merged. A real Contract-bound Draft PR must pass CI and produce a matching receipt; only then may the row be reconsidered. Continuous watcher/catalog remains conditionally deferred. |
