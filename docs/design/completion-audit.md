...
| Stable model surface | Compact search, exact get, governed remember, separate temporary board | shared `src/integration/agent-surface.ts` projections/renderers, shared action contract, Pi/MCP/DSH thin registrations, OmniMemEval exact-evidence projection and cross-adapter contract tests | **Verified core** | Monitor host schema drift in real harness releases; semantic result changes belong in the shared surface first. |
