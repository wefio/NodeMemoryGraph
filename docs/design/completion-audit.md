...
| Repository Control Plane | External Contract-to-Agent-to-Code-to-Verifier reconciliation with separate desired, observed, receipt and forge truth domains; optional one-way NMG integration | normative target in `ci-cd-and-quality.md` and paired proposed decision; existing context/verify/CI/Task Board are reusable primitives only | **Designed target** | Contract IR/compiler, observer/reconciler, WorkOrder, independent verifier receipt, forge/providers and no-NMG closed-loop proof are not implemented. |
