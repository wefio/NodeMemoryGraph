...
7. **实际使用投影独立限额**：共享 `src/integration` 投影负责链名、本次响应短标签、fan-in/fan-out 压缩和结构预算；Pi 与 Claude 的搜索头提示逻辑链可用并标记同链候选，精确 `get` 才加载证据和所选证据之间的边。CLI 紧凑搜索暴露链归属和数量，精确 JSON 保留 canonical memberships/edges。每条正文只出现一次；结构默认最多 2048 字符，超限可折叠结构但不能挤掉精确证据。OmniMemEval 的独立 renderer 仅服从评测格式，不定义产品契约。时间链继续使用记录时间字段，不重复生成时间线。
...
- ✅ **Agent 实际投影**：搜索头提示链可用；精确 `get` 在 Core 和 STG/LTG 合并后保留 memberships/edges；共享投影由 Pi 与 Claude 消费，CLI 暴露结构化链元数据，正文只出现一次且结构使用独立预算。
