...
memory-graphs.md §1 — STG is a _semantic lifecycle_ (provisional,
...
- **Never promoted back**: a `cached_from_ltg` memory must not enter the
  promotion pipeline (it already _is_ LTG — promoting it would be a copy
...
2. **Cached LTG subset** (`cached_from_ltg`): copied from LTG by a
   **usage-driven** routine — not by blindly copying global L1/L2. The AG
   ...
   actually _used_; the routine copies those (per project) into its STG.
