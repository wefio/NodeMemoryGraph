...
| 3 | Router expressiveness | **Mechanism and causal runner complete; quality not proven.** The custom autodiff controller learns node, memory, edge, control, and budget heads. A frozen candidate can now actuate bounded node reranking, allocation, and folding in matched Pi runs, with typed action telemetry and fail-closed official scoring. One smoke proves the path; a sufficiently sized evidence-diverse held-out comparison is still required before promotion. |
