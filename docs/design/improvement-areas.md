...
| 3 | Router expressiveness | **Superseded experimentally, not proven.** The custom autodiff controller now learns node, memory, edge, control, and budget heads. Same-case official scoring and Pi tool/token/latency aggregation are wired fail-closed, but the current shadow arm does not affect ranking and therefore still provides no production-quality matched quality/cost win. |
