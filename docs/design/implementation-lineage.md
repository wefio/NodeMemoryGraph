...
| Session Active Graph runtime (daemon-owned working memory) | **Introduced** protocol v9 + `SessionActiveGraphRuntime` core; **Hardened** task-frame/branch lifecycle, unified session-wide budget, TTL reasoning artifacts, disclosure ledger | [Session AG runtime](session-active-graph-runtime-design.md), [AG runtime decision](../decisions/implemented/2026-08-29-session-active-graph-runtime.md) |
| File content source for search | **Proposed** (2026-09-01): bounded passive scan of `.nmg-search-scope` hot zones; scope learned from Agent grep/read behaviour; lexical-first FTS index; memory+file fusion | [File content source](archived/file-content-source-design.md) |
