...
export const GENERATED_NMG_PROMPTS = {
  ...
  reason_action_parameter_description:
    "add creates one typed scratch node and idempotently reuses an exact same-kind, same-content node; update is required for semantic change; link records one explicit relation; checkpoint returns a bounded current view; clear deletes only this session's reasoning scratchpad.",
  reason_node_id_parameter_description:
    "Stable reasoning-node ID returned by an earlier nmg_reason call. Required for update and scoped to the current Pi session.",
  reason_kind_parameter_description:
    "The role of a new scratch node in the current reasoning process. Required for add; it is not a durable MemoryNode type.",
  reason_content_parameter_description:
    "Concise scratch content for add, or replacement content for update. Keep uncertainty explicit; scratch content is not verified fact.",
  reason_status_parameter_description:
    "Epistemic or workflow status of the scratch node. supported means the node has traceable support; rejected preserves a disproven path; resolved and superseded keep lifecycle history.",
  reason_importance_parameter_description:
    "Session-local checkpoint priority from 0 to 1. It affects bounded disclosure, not truth, durability, or automatic promotion.",
  reason_evidence_refs_parameter_description:
    "Stable references supporting a new or updated scratch node, such as NMG memory IDs, source-message IDs, file locations, or tool-result identities. Required for evidence nodes and for directly supported nodes; alternatively mark a node supported only after linking an already anchored source with supports or derived_from. References are retained for audit but are not independently verified by this tool.",
  reason_source_id_parameter_description:
    "Existing reasoning-node ID at the source of a relation. Required for link.",
  reason_target_id_parameter_description:
    "Existing reasoning-node ID at the target of a relation. Required for link.",
  reason_relation_parameter_description:
    "Explicit directed relation between two existing scratch nodes. Required for link and scoped to the current Pi session.",
  ...
  remember_memory_id_parameter_description:
    "For action=forget, resolve, or reopen, the exact target memory ID. This creates an auditable logical deletion, not physical privacy erasure.",
  remember_new_memory_id_parameter_description:
    "For action=supersede, the new memory ID returned by the preceding save.",
  remember_superseded_memory_id_parameter_description:
    "For action=supersede, the candidate memory ID whose value is replaced. Do not use this merely because two memories are related or similar.",
  remember_related_memory_id_parameter_description:
    "For action=relate, a candidate memory ID whose node has a meaningful semantic relationship with the newly saved memory. This does not merge either node.",
  remember_relation_judgement_parameter_description:
    "For action=relate, classify the nodes as same_entity, related, refines, conflict, or distinct. Use same_entity only for identity, not similarity; use distinct when an apparent match is actually a different entity or scope.",
  node_name_parameter_description:
    "Stable semantic cluster name. Related memories may share a node even when they describe different properties.",
  recall_triggers_parameter_description:
    "Optional short aliases or likely query phrases that should recall this record when the saved statement uses different wording. Use only distinctive wording a future user or Agent may actually search for; do not copy the whole statement, add broad topic tags, or encode new facts here. At most 16 entries.",
  ...
  external_source_parameter_description:
    "External provenance reference beginning with web: or file:.",
  evidence_parameter_description:
    "Smallest exact excerpt that supports the saved statement. Copy it from the user, assistant, or tool source identified by sourceActor; do not summarize or include surrounding routine prose. Pi binds a matching excerpt to its source message. Use externalSource for web/file evidence.",
  source_actor_parameter_description:
    "Author of the supporting evidence, not the desired authority of the memory. Omit only for Assistant-created content (the safe default). Use user, tool, or system only with an exact evidence excerpt from that source in the current Pi session, or with an explicit externalSource for tool-derived web/file evidence. Never mark an Assistant inference or summary as sourceActor=user.",
  active_graph_id_parameter_description:
    "Optional activeGraphId returned by nmg_search. For action=feedback, omit it to target the latest retrieval owned by the current Pi session; pass it to label a specific older retrieval. For nmg_get, pass it when available to attribute use.",
  mcp_active_graph_id_parameter_description:
    "Optional activeGraphId returned by nmg_search. Pass it to nmg_get when available so exact disclosure is attributed to this MCP session, or to claim_outcome when the selected claim came from that retrieval.",
  feedback_note_parameter_description:
    "Optional concise reason for an explicit retrieval/task label. Do not include hidden reasoning, secrets, or infer success merely because no correction was made.",
  feedback_label_parameter_description:
    "Explicit observed label for action=feedback. Omit when unknown; false means the condition was actually reviewed and absent, not merely that nobody said it.",
  semantic_task_id_parameter_description:
    "Optional stable identifier shared by retries of the same semantic task. It is used for calibration deduplication, not session authorization or memory scope.",
  ...
  search_recommendation:
    "Automatic recall may be incomplete. If prior experience could change the answer, make one deliberate nmg_search call and then inspect only useful IDs with nmg_get.",
  completion_nudge:
    "A code commit or task completion was just detected. NMG long-term memory is available: you may recall relevant decisions (nmg_search) or save this turn's key conclusions (nmg_remember) if useful. This is only a reminder; it does not affect the current work.",
  shadow_feedback_nudge:
  ...
  get_hint:
    "Use nmg_get with selected memory IDs and the activeGraphId to load exact records and source evidence.",
  ...
  claim_outcome_parameter_description:
    "Explicit result for selected saved claims: supported or contradicted. This is evidence about the claim itself, not retrieval quality. Never infer support merely because a memory was retrieved, used in an answer, or not corrected.",
  claim_outcome_source_parameter_description:
    "Attribution class for the outcome: user, tool, or task. User/tool outcomes require an exact evidence excerpt from the current Pi session; task outcomes require claimSourceLineage. Benchmark outcomes are reserved for eval/CLI.",
  claim_source_lineage_parameter_description:
    "Stable identity of an independently attributable completed task. For user/tool outcomes Pi derives lineage from the exact evidence message instead of trusting a model-provided ID. Reusing lineage prevents one source from becoming multiple independent votes.",
  claim_indexes_parameter_description:
    "Optional zero-based indexes of atomic claims within the memory. Omit to apply the explicit outcome to every claim in that memory.",
} as const;
