...
function temporalLabels(candidate) {
...
function renderCompactSearchSurface(context, options = {}) {
	...
	const lines = context.candidates.map((candidate) => {
		const flags = [candidate.external ? "[external]" : "", candidate.open ? "[open]" : ""].filter(Boolean);
		const fields = [
			`memory=${candidate.id}`,
			`node=${candidate.node}`,
			`type=${candidate.type}`,
	...
	return [
		options.preamble,
		...lines,
		context.logicalChainCount > 0 ? `logical_chains=${context.logicalChainCount}; use nmg_get for compact chain structure with exact evidence.` : "",
...
function renderEvidenceSurface(context, options = {}) {
	...
	const records = context.results.map(({ memory, node, evidence }) => {
		...
		const source = !forgotten && evidence.content.trim() !== memory.statement.trim() ? `\n  SOURCE=${excerpt(evidence.content, 320)}` : "";
...
//#endregion
//#region ../../src/prompts/nmg-prompts.generated.ts
const GENERATED_NMG_PROMPTS = {
	search_description: "Search long-term memory. Queries match saved records by literal terms, semantic similarity, and learned graph routes; returns ranked candidate headers (memory id, node, type, match terms, event date, preview) plus an activeGraphId. Exact records and source evidence are loaded via nmg_get. The store only contains what was explicitly saved - it is not a live source. Advanced syntax: key:value filters (type:preference,constraint node:\"Conversation ...\" state:key time:2026-01-01..2026-06-30), -term exclusions, and quoted phrases.",
	get_description: "Load exact memory records and source evidence for memory IDs returned by nmg_search (or nmg_automatic_recall). When available, pass the activeGraphId returned by search so NMG can attribute actual use; it is not required to read a known memory ID. Returns the full statement plus evidence content and source reference.",
	remember_description: "Save a durable record (fact, state, event, preference, constraint, strategy), resolve a possible supersession, attach explicit evaluation feedback to a retrieval trace, or record an attributable supported/contradicted outcome for selected saved claims. You decide the semantic content and whether a candidate is truly an older value; NMG validates scope, identity, provenance, and applies the transaction. The record persists across sessions. Only save attributable information - not secrets, transient content, or unsupported guesses. Never label an Assistant inference as a user statement. A stateKey names one replaceable property within a stable scope; it is not a topic or grouping tag. recallTriggers may name a few short aliases or likely query phrases that should retrieve this record when its factual statement uses different wording.",
	mcp_remember_description: "Save a durable record or apply an explicit memory lifecycle action. Supports save, supersede, relate, forget, resolve, reopen, and task-attributed claim_outcome. The MCP adapter cannot verify user/tool message evidence and does not expose Pi-local retrieval feedback, so claim_outcome accepts only a task source with an explicit source lineage. NMG validates scope, identity, provenance, and applies the transaction.",
	board_description: "Coordinate through a temporary blackboard shared by NMG clients. Without a taskId, entries land on the shared world channel; reading it lists active named channels you can join by name. Entries expire automatically and retain writer attribution (NMG_AGENT_ID). Use put/read/resolve; full conventions (memory pointers, boundaries) are disclosed in each tool result.",
	lab_description: "Discover and temporarily enable optional NMG capabilities for the current session. Use list before first use, enable only when the current task benefits, invoke a documented operation, and disable when finished. Reasoning workspace, graph reasoner, and controller shadow are self-service; controlled or active controller modes remain harness/operator gated. Lab output is provisional and never becomes durable memory unless separately submitted through nmg_remember.",
	reason_description: "Maintain a session-private, auditable reasoning scratchpad containing typed goals, observations, hypotheses, evidence, conclusions, decisions, open questions, next actions, and explicit relations. It persists checkpoints across Pi compaction and process restart but never writes scratch content into durable semantic memory. Evidence nodes require stable source references; supported nodes require either such a reference or an anchored support path. Unsupported hypotheses remain visibly marked as unsupported after compaction.",
	reason_action_parameter_description: "add creates one typed scratch node and idempotently reuses an exact same-kind, same-content node; update is required for semantic change; link records one explicit relation; checkpoint returns a bounded current view; clear deletes only this session's reasoning scratchpad.",
	reason_node_id_parameter_description: "Stable reasoning-node ID returned by an earlier nmg_reason call. Required for update and scoped to the current Pi session.",
	reason_kind_parameter_description: "The role of a new scratch node in the current reasoning process. Required for add; it is not a durable MemoryNode type.",
	reason_content_parameter_description: "Concise scratch content for add, or replacement content for update. Keep uncertainty explicit; scratch content is not verified fact.",
	reason_status_parameter_description: "Epistemic or workflow status of the scratch node. supported means the node has traceable support; rejected preserves a disproven path; resolved and superseded keep lifecycle history.",
	reason_importance_parameter_description: "Session-local checkpoint priority from 0 to 1. It affects bounded disclosure, not truth, durability, or automatic promotion.",
	reason_evidence_refs_parameter_description: "Stable references supporting a new or updated scratch node, such as NMG memory IDs, source-message IDs, file locations, or tool-result identities. Required for evidence nodes and for directly supported nodes; alternatively mark a node supported only after linking an already anchored source with supports or derived_from. References are retained for audit but are not independently verified by this tool.",
	reason_source_id_parameter_description: "Existing reasoning-node ID at the source of a relation. Required for link.",
	reason_target_id_parameter_description: "Existing reasoning-node ID at the target of a relation. Required for link.",
	reason_relation_parameter_description: "Explicit directed relation between two existing scratch nodes. Required for link and scoped to the current Pi session.",
	board_action_parameter_description: "Use put to publish one concise coordination entry, read to fetch entries after an optional cursor, claim to take an open entry as the single working Agent (lease-based; expired claims return to the pool, resolving clears the claim), release to put a claimed entry back, unsubscribe to leave a channel (stop receiving its wake notices; the world channel is your default member channel and can be muted the same way), subscribe to join a channel (topic-based membership — a named channel only wakes its members, never non-members), acknowledge to record that you have seen and accepted an entry and owe no reply (pure state write — no broadcast, no wake, and the acked entry stops re-notifying you), or resolve to close a selected entry. discover lists online agents (A2A roster, by capability) so you can pick a target, then put ... to=<stable agent_name> for directed delivery — only that agent's LLM is woken, everyone else reads but stays silent (find first via discover, then direct). Un-directed actionable entries (handoff/question/ blocker) are serialised per channel: only one outstanding is pushed at a time; later ones sit 'pending' (read-visible, silent) until the outstanding one is claimed (接手=回复) or resolved, which promotes the next. Directed entries are exempt (parallel-safe). kind decides wake routing: question/blocker/handoff ask for a response and WAKE subscribers; note/result/decision/goal are notify-only and SILENT (they record a fact, owe no reply, and are read on demand — never pushed). If you need an answer or action, use question/handoff; never smuggle a request into a note (a silent note asking for something starves). When a fact is already confirmed, do not post a confirming note — acknowledge it (one ack is enough; everyone confirming one fact is an acknowledgement storm). Confirmations close: once a question/handoff is answered, resolve it — a resolved entry is closed and should not be replied to (reopen only for new substance). Acknowledgement is the lightweight \"确认但不用回\": for a result/note you accept, prefer acknowledge over posting a new reply entry — acknowledgers are visible in read output (✅ acked by ...) and readers check them on demand instead of being broadcast every Ack. Put/read/claim/release/ resolve/acknowledge do not create semantic memory. To persist a board conclusion as durable memory WITHOUT duplicating it: (1) nmg_search first — if the fact already exists as a memory, do not\n    remember it again; point the board entry at it with memory=<id>.\n(2) only if no existing memory covers it, nmg_remember with\n    boardSource: {taskId, entryId} (records a board_origin marker so the\n    memory traces back to this board entry and later dedup can recognize\n    it). (3) resolve the board entry once the memory is created, noting\n    memory=<id> in the resolution.",
	remember_board_source_parameter_description: "Provenance link from a task board entry to the durable memory being created from its content. When set, nmg_remember attaches a board_origin marker ({kind: \"board_origin\", attributes: {taskId, entryId}}) so the memory traces back to the board entry and later writes of the same board content can be recognized as duplicates. Use it ONLY when creating a NEW memory from board content that has no existing memory counterpart (search first).",
	board_task_id_parameter_description: "Optional. Name of the channel to read or write. When omitted, entries land on the shared world channel (the default), which every Agent reads; reading the world channel also lists active named channels so you can discover and join one by name.",
	board_content_parameter_description: "Concise task coordination state such as a goal, blocker, result, handoff, question, or decision. Do not include secrets or hidden chain-of-thought. To point other Agents at durable knowledge, include memory=<id> references (the same format automatic recall uses); readers expand them with nmg_get.",
	remember_action_parameter_description: "Omit or use save for a new memory. Use supersede only when a candidate is the older value of the new memory. Use relate to propose a semantic relationship without merging node identity; the proposal remains reversible and pending. Use forget only when the user explicitly asks to withdraw a selected memory. Use resolve when an open question or dependency has been settled. Use reopen when later evidence makes a resolved memory actionable again; provide the related evidence memory IDs so the open structure stays attributable. Use feedback to record explicit task/retrieval labels. Use claim_outcome only after an attributable user, tool, or completed task explicitly supports or contradicts selected saved claims; retrieval, use, silence, and task success alone are not claim outcomes. Pass activeGraphId when labelling a specific older retrieval; omit it only to label the latest retrieval in the current Pi session. Silence or lack of correction is not positive feedback.",
	mcp_remember_action_parameter_description: "Omit or use save for a new memory. Use supersede only when the selected older value has the same scope, relate to create a reversible semantic proposal, forget only for an explicit withdrawal, and resolve or reopen for an open memory lifecycle change. Use claim_outcome only for an independently attributable completed task, with semanticTaskId and claimSourceLineage. The MCP adapter does not accept Pi-local retrieval feedback or user/tool claim evidence because it cannot verify those current-session message sources.",
	remember_memory_id_parameter_description: "For action=forget, resolve, or reopen, the exact target memory ID. This creates an auditable logical deletion, not physical privacy erasure.",
	remember_new_memory_id_parameter_description: "For action=supersede, the new memory ID returned by the preceding save.",
	remember_superseded_memory_id_parameter_description: "For action=supersede, the candidate memory ID whose value is replaced. Do not use this merely because two memories are related or similar.",
	remember_related_memory_id_parameter_description: "For action=relate, a candidate memory ID whose node has a meaningful semantic relationship with the newly saved memory. This does not merge either node.",
	remember_relation_judgement_parameter_description: "For action=relate, classify the nodes as same_entity, related, refines, conflict, or distinct. Use same_entity only for identity, not similarity; use distinct when an apparent match is actually a different entity or scope.",
	node_name_parameter_description: "Stable semantic cluster name. Related memories may share a node even when they describe different properties.",
	recall_triggers_parameter_description: "Optional short aliases or likely query phrases that should recall this record when the saved statement uses different wording. Use only distinctive wording a future user or Agent may actually search for; do not copy the whole statement, add broad topic tags, or encode new facts here. At most 16 entries.",
	state_key_parameter_description: "Stable key for exactly one replaceable property within one canonical scope. Reuse the key when the new value makes the old value no longer current; for example, running.charity_5k.personal_best changes from 27:12 to 25:50. Use different keys when both values can remain true; for example, a personal best and a target time are distinct properties. Good: pi-lsp.installation.path. Bad: pi-lsp-env used for installation path, tool inventory, and patch policy. A newer state with the same key and canonical scope automatically supersedes the previous active state, so a broad or wrongly reused key can incorrectly retire an unrelated state.",
	external_source_parameter_description: "External provenance reference beginning with web: or file:.",
	...
	source_actor_parameter_description: "Author of the supporting evidence, not the desired authority of the memory. Omit only for Assistant-created content (the safe default). Use user, tool, or system only with an exact evidence excerpt from that source in the current Pi session, or with an explicit externalSource for tool-derived web/file evidence. Never mark an Assistant inference or summary as sourceActor=user.",
	active_graph_id_parameter_description: "Optional activeGraphId returned by nmg_search. For action=feedback, omit it to target the latest retrieval owned by the current Pi session; pass it to label a specific older retrieval. For nmg_get, pass it when available to attribute use.",
	mcp_active_graph_id_parameter_description: "Optional activeGraphId returned by nmg_search. Pass it to nmg_get when available so exact disclosure is attributed to this MCP session, or to claim_outcome when the selected claim came from that retrieval.",
	feedback_note_parameter_description: "Optional concise reason for an explicit retrieval/task label. Do not include hidden reasoning, secrets, or infer success merely because no correction was made.",
	feedback_label_parameter_description: "Explicit observed label for action=feedback. Omit when unknown; false means the condition was actually reviewed and absent, not merely that nobody said it.",
	semantic_task_id_parameter_description: "Optional stable identifier shared by retries of the same semantic task. It is used for calibration deduplication, not session authorization or memory scope.",
	search_query_parameter_description: "Natural-language retrieval clause; may include key:value filters, -term exclusions, and quoted phrases.",
	search_queries_parameter_description: "Optional Agent-generated retrieval clauses, such as query decomposition, alternate wording, or a hypothetical-answer (HyDE) clause. NMG does not generate these clauses; it searches them, removes duplicate records, and appends unique results after the primary query's ranking.",
	search_progression_required: "NMG paused another search because two searches were already completed without loading selected evidence. Use nmg_get on relevant returned memory IDs, or use a current-source tool when the missing fact concerns live code, files, or the web. Do not keep paraphrasing the same historical search.",
	search_recommendation: "Automatic recall may be incomplete. If prior experience could change the answer, make one deliberate nmg_search call and then inspect only useful IDs with nmg_get.",
	completion_nudge: "A code commit or task completion was just detected. NMG long-term memory is available: you may recall relevant decisions (nmg_search) or save this turn's key conclusions (nmg_remember) if useful. This is only a reminder; it does not affect the current work.",
	shadow_feedback_nudge: "The previous NMG retrieval activeGraphId={active_graph_id} has completed and remains unlabelled. If the preceding answer and tool evidence let you review it, call nmg_remember action=feedback with semanticTaskId={semantic_task_id} and explicit values for evidenceSufficient, expansionUseful, excessiveNoise, and noMemoryNeeded. Omit the feedback call when any required label would be a guess. Silence, task completion, and lack of user correction are not labels.",
...
*/
function renderDisclosure(template, vars) {
	return template.replace(/\{(\w+)\}/g, (_match, key) => vars[key] ?? _match).split("\n").filter((line) => line.trim().length > 0).join("\n");
}
//#endregion
...
function apply(ctx) {
	...
	function formatRecall(recall, candidates) {
		...
		const nextStep = deferred.length ? nmgPrompts.deferred_hint + " Memory IDs: " + deferred.join(",") : nmgPrompts.get_hint;
	...
	let wakeConfig = null;
	const lastAgents = /* @__PURE__ */ new Map();
	...
	const searchTool = {
		...
		async execute(args, exec) {
			...
			const nextStep = Array.isArray(deferred) && deferred.length ? "More ranked records are folded. Expand selected memory IDs first; deferred IDs: " + deferred.join(",") : "Select exact records with nmg_get (memory IDs + activeGraphId).";
			const forget = data.results.some((result) => (result.memory.markers || []).some((marker) => marker.kind === "forget"));
	...
	const getTool = {
		...
		async execute(args, exec) {
			...
			if (!r.ok) return r.error;
			const forget = r.data.results.some((result) => (result.memory.markers || []).some((marker) => marker.kind === "forget"));
